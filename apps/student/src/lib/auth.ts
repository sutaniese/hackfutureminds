...
/**
 * Local-storage based account system (Variant A).
 *
 * Storage layout:
 *   pathwise-auth-users    -> Record<emailLower, StoredUser>
 *   pathwise-auth-session  -> { email: emailLower } | null
 *
 * Passwords are never stored in plaintext: we derive a key with PBKDF2-SHA256
 * (200k iterations, 16-byte salt) and store salt + hash as base64.
 *
 * This is intentionally browser-only — there is no remote backend yet.
 * It is "good enough" to:
 *   - keep accounts isolated between people sharing a browser
 *   - avoid leaking the literal password into devtools / extensions
 *   - tie a chosen role to an account
 *
 * It is NOT a substitute for server-side auth and should not be treated as one.
 */
