...
Ты опираешься ТОЛЬКО на контекст из его Obsidian-vault (профиль и заметки), который тебе передаётся ниже.
...
Если пользователь просит "запомнить", "сохрани в Obsidian", "добавь заметку" — выдай команду
...
function buildFallback(name: string, message: string): string {
  return [
    ...
    'Когда подключим API — отвечу с учётом профиля и заметок из Obsidian-vault.',
  ].join('\n')
}
